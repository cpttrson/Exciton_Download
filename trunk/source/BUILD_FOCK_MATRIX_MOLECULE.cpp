...
void build_fock_matrix_molecule_no_sym(double *Fock, INT_1E *one_ints, FERMI *fermi, double *total_electrons, double *S1, double *P, double *F, double *delta_P, double *delta_F, PAIR_TRAN *pair_p, ATOM_TRAN *atom_p, ATOM *atoms, SHELL *shells, GAUSSIAN *gaussians, ATOM *atoms_ax, SHELL *shells_ax, GAUSSIAN *gaussians_ax, CRYSTAL *crystal, SYMMETRY *symmetry,REAL_LATTICE *R, REAL_LATTICE_TABLES *R_tables, RECIPROCAL_LATTICE *G, JOB_PARAM *job, FILES file)
...
//job->int_exist_no_sym = 1;
  ...
  */
  ...
  if (job->int_exist_no_sym == 0 || job->scf_direct == 1) 
   ...
   fock_matrix_molecule_compute_integrals_no_sym(Fock_2c,Fock_2e,S1,F,pair_p,&pair_q,atoms,shells,gaussians,\
...
MPI_Win win;
  ...
  if (job->taskid > 0 || job->numtasks == 1) while (myCounter < total_tasks) {
    ...
    for (q = begin_task; q < end_task; q++) {
      ...
      time8 = MPI_Wtime();
      count = 0; 
      integrals_crystal_ijkl3(&integral_list,&Quad,start_index,&count,R,G,atoms,shells,gaussians,symmetry,crystal,job,file);
      total_integrals += integral_list.num;
      time7 += MPI_Wtime() - time8;
      time10 = MPI_Wtime();
      contract_coulomb_integrals(Fock_2c_buffer,&integral_list,F,pair_p,&Quad,atoms,shells,symmetry,R_tables,job,file);
      time9 += MPI_Wtime() - time10;

      time12 = MPI_Wtime();
      //if (job->scf_direct == 0 && integral_list.num > 0 && (job->taskid > 0 || job->numtasks == 1))
      if (integral_list.num > 0 && (job->taskid > 0 || job->numtasks == 1))
      pack_write_coulomb_2e_integrals(&integral_list, &Quad, integrals_2c, job, file);
      time11 += MPI_Wtime() - time12;
     }
...
MPI_Win win;
  ...
  time9 += MPI_Wtime() - time10;
  time12 = MPI_Wtime();
  contract_exchange_integrals4(Fock_2e_buffer,&integral_list,F,pair_p,&quad,atoms,shells,symmetry,R_tables,job,file);
  time11 += MPI_Wtime() - time12;
  free_integral_list(&integral_list,job);
  } // close if (quad.tot > 0)
...
char xx[4], yy[24] = "scf_integrals_2C.";
  ...
  QUAD_TRAN Quad;
  INTEGRAL_LIST integral_list;
  fread(&Quad.tot,sizeof(int),1,integrals_2c);
  fread(&integral_list.num,sizeof(int),1,integrals_2c);
  if (feof(integrals_2c)) break;
  allocate_QUAD_TRAN(&Quad,job,file);
  allocate_integral_list(&integral_list, integral_list.num, job, file);
  read_unpack_coulomb_2e_integrals(&integral_list, &Quad, integrals_2c, job, file);
  contract_coulomb_integrals(Fock_2c_buffer,&integral_list,F,pair_p,&Quad,atoms,shells,symmetry,R_tables,job,file);
  free_QUAD_TRAN(&Quad,job);
  free_integral_list(&integral_list,job);
 } //while (!feof
...
void exchange_matrix_crystal_read_integrals(double *Fock_2e, double *F, PAIR_TRAN *pair_p, ATOM_TRAN *atom_p, ATOM *atoms, SHELL *shells, GAUSSIAN *gaussians, CRYSTAL *crystal, SYMMETRY *symmetry, REAL_LATTICE *R, REAL_LATTICE_TABLES *R_tables, JOB_PARAM *job, FILES file)
...
char xx[4], yy[24] = "scf_integrals_2E.";
  ...
  QUAD_TRAN Quad;
  INTEGRAL_LIST integral_list;
  fread(&Quad.tot,sizeof(int),1,integrals_2e);
  fread(&integral_list.num,sizeof(int),1,integrals_2e);
  if (feof(integrals_2e)) break;
  allocate_QUAD_TRAN(&Quad,job,file);
  allocate_integral_list(&integral_list, integral_list.num, job, file);
  read_unpack_coulomb_2e_integrals(&integral_list, &Quad, integrals_2e, job, file);
  contract_exchange_integrals4(Fock_2e_buffer,&integral_list,F,pair_p,&Quad,atoms,shells,symmetry,R_tables,job,file);
  free_QUAD_TRAN(&Quad,job);
  free_integral_list(&integral_list,job);
...
char xx[4], yy[24] = "scf_integrals_2E.";
  ...
  allocate_QUAD_TRAN(&Quad,job,file);
  allocate_integral_list(&integral_list, integral_list.num, job, file);
  read_unpack_exchange_2e_integrals(&integral_list, &Quad, integrals_2e, job, file);
  atm_n1 = Quad.cell1[0];
  atm_n2 = Quad.cell2[0];
  atm_n3 = Quad.cell3[0];
  atm_n4 = Quad.cell4[0];
