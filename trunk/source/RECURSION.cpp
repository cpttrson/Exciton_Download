...

{
...

  {
    recurrence = (t >= 0 && t <= i + ip) ? (e(i - 1, ip, t - 1, p, PAx, PBx) / 2 / p + PAx * e(i - 1, ip, t, p, PAx,
        PBx) + (t + 1) * e(i - 1, ip, t + 1, p, PAx, PBx)) : 0.0;
  }
...

  {
    recurrence = (t >= 0 && t <= i + ip) ? (e(i, ip - 1, t - 1, p, PAx, PBx) / 2 / p + PBx * e(i, ip - 1, t, p, PAx,
        PBx) + (t + 1) * e(i, ip - 1, t + 1, p, PAx, PBx)) : 0.0;
  }
...

{
  ...
  double result;
  //double mag = x.comp1*x.comp1 + x.comp2*x.comp2 + x.comp3*x.comp3 ;
  if (t < 0 || u < 0 || v < 0)
    return 0.0;
  //if ((t > 0 || u > 0 || v > 0) && mag < 0.00001) return 0.0 ;
  if (t >= u && t >= v) {
          //printf("t %d %d %d %d \n",t,u,v,n) ;
...
//CHANGES2015void non_recursive_ftuvn(int mm, int index_R, double f[][9][9][9], double en[][55], VECTOR_DOUBLE *r_12)
