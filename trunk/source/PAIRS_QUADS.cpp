...
VECTOR_DOUBLE Rvec_tmp;
  ...
  if (job->taskid >= 0 && (job->verbosity > 1 || job->print_pairs == 1)) {
    ...
    fprintf(file.out,"\n\n===========================================================================================================\n");
    fprintf(file.out,"|                                          PAIR LIST                                                      |\n");
    fprintf(file.out,"|---------------------------------------------------------------------------------------------------------|\n");
    fprintf(file.out,"| UNIQUE PAIR               | EQUIVALENT PAIRS        | TRANSFORMATION          | PAIR SEPARATION (Angs.) |\n");
    fprintf(file.out,"|---------------------------------------------------------------------------------------------------------|\n");
    fprintf(file.out,"|                           |                         |                         |                         |\n");
    ...
    for (i = 0; i < pair_p->nump; i++) {
      ...
      for (j = 0; j < pair_p->numb[i]; j++) {
        ...
        fprintf(file.out,"| pair[%5d]  %3d %3d  %3d | pair[%5d] %3d %3d %3d | operator %2d permut'n %2d |              %10.4lf |\n",\
        q, pair_p->cell1[q], pair_p->cell2[q], pair_p->latt2[q], q + j, pair_p->cell1[q + j], pair_p->cell2[q + j], \
        pair_p->latt2[q + j],pair_p->k[q + j],pair_p->p[q + j],sqrt(RAB2) * bohr_to_AA);
       }
        fprintf(file.out,"|                           |                         |                         |                         |\n");
