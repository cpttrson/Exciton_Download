...
{
  double ang_x, fact;
  *a=acos(z); // theta, phi were switched in original version CHP 02/02/2011
  //*b=acos(z);
  //b=acos(z);
...
int Lebedev_Laikov_npoint(int lvalue, FILES file)
{
  ...
  if (lvalue <3) {
      ...
      fprintf(file.out," Lebedev_Laikov_npoint: lvalue > 131.  No grids of this type available.\n");
      abort();
    }
  return 0;
}
...
int Lebedev_grid (double *theta, double *phi, double *grid_x, double *grid_y, double *grid_z, double *weights, int *M, int N, FILES file)
{
  int i, j, k, m;
  //int i, j, k, m, M, N;
  int MMAX = (N*2+3)*(N*2+3)/3;
  ...
  double x[MMAX], y[MMAX], z[MMAX], w[MMAX], alpha[MMAX],beta[MMAX];
  double xn[MMAX*(NMAX+1)], yn[MMAX*(NMAX+1)], zn[MMAX*(NMAX+1)];
  static double s[NMAX+2];
  double S, S0, r, rmax;
  //for (N = 1; N <= NMAX; N++) {
    *M = NW[N];
    //M = NW[N];
    Lebedev_Laikov_sphere(*M, grid_x, grid_y, grid_z, weights);
    Lebedev_Laikov_sphere(*M, x, y, z, w);
