...

{
  ...
  job->overlap_tol_1 = pow(10.00,-10);
  job->overlap_tol_2 = pow(10.00,-5);
  job->overlap_tol_3 = pow(10.00,-5);
  job->overlap_tol_4 = pow(10.00,-5);
  job->overlap_tol_5 = pow(10.00,-5);
  job->overlap_tol_6 = pow(10.00,-5);
  job->overlap_tol_7 = pow(10.00,-5);
  job->overlap_tol_8 = pow(10.00,-5);
  job->overlap_tol_9 = pow(10.00,-5);
...
/*
May2013
  job->pms     = 1;             // default is permutation symmetry on
  job->trs     = 1;             // default is time reversal symmetry on
  job->sgs     = 1;             // default is space group symmetry on
*/

  //job->C09     = 0;             // default is do not read from Crystal09
  ...
  //fprintf(file.out, "\n");
  //fprintf(file.out, "************************************************************\n");
  //fprintf(file.out, "                       STARTING ANALYSIS \n");
  //fprintf(file.out, "************************************************************\n\n");
  ...
  if (job->taskid == 0)
      fprintf(file.out,"K SPACE SYMMETRY SWITCHED OFF\n\n");

    }

    /*****JOB: LATTICE SUM TOLERANCES ***************************************************/

    if (!strcmp(jobname, "LATTICE_SUM_TOLERANCES")) {

      double tol_1, tol_2, tol_3, tol_4, tol_5, tol_6, tol_7, tol_8, tol_9;

      read_line(file.job, title, 99);
      sscanf(title, "%lf %lf %lf %lf %lf %lf %lf %lf %lf", &tol_1, &tol_2, &tol_3, &tol_4, &tol_5, &tol_6, &tol_7, &tol_8, &tol_9);

      job->overlap_tol_1 = pow(10.00,-tol_1);
      job->overlap_tol_2 = pow(10.00,-tol_2);
      job->overlap_tol_3 = pow(10.00,-tol_3);
      job->overlap_tol_4 = pow(10.00,-tol_4);
      job->overlap_tol_5 = pow(10.00,-tol_5);
      job->overlap_tol_6 = pow(10.00,-tol_6);
      job->overlap_tol_7 = pow(10.00,-tol_7);
      job->overlap_tol_8 = pow(10.00,-tol_8);
      job->overlap_tol_9 = pow(10.00,-tol_9);

      if (job->overlap_tol_1 < 1.0e-24 || job->overlap_tol_1 > 1.0e-03 || job->overlap_tol_2 < 1.0e-24 || job->overlap_tol_2 > 1.0e-03 || 
          job->overlap_tol_3 < 1.0e-24 || job->overlap_tol_3 > 1.0e-03 || job->overlap_tol_4 < 1.0e-24 || job->overlap_tol_4 > 1.0e-03 || 
          job->overlap_tol_5 < 1.0e-24 || job->overlap_tol_5 > 1.0e-03 || job->overlap_tol_6 < 1.0e-24 || job->overlap_tol_6 > 1.0e-03 ||
          job->overlap_tol_7 < 1.0e-24 || job->overlap_tol_7 > 1.0e-03 || job->overlap_tol_8 < 1.0e-24 || job->overlap_tol_8 > 1.0e-03 ||
          job->overlap_tol_9 < 1.0e-24 || job->overlap_tol_9 > 1.0e-03) {
      if (job->taskid == 0)
      fprintf(file.out,"ERROR: LATTICE TOLERANCES MUST BE IN RANGE 1.0E-24 to 1.0E-03 VALUE ENTERED \
      %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e\n",\
      job->overlap_tol_1,job->overlap_tol_2,job->overlap_tol_3,job->overlap_tol_4,job->overlap_tol_5,job->overlap_tol_6,job->overlap_tol_7,\
      job->overlap_tol_8,job->overlap_tol_9);
      MPI_Finalize();
      exit(1);
     }

      //if (job->taskid == 0)
      //fprintf(file.out,"LATTICE SUM TOLERANCES %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e\n\n",job->overlap_tol_1, \
      job->overlap_tol_2,job->overlap_tol_3,job->overlap_tol_4,job->overlap_tol_5,job->overlap_tol_6,job->overlap_tol_7,\
      job->overlap_tol_8,job->overlap_tol_9);
    ...
    if (!strcmp(jobname, "SPIN_POLARISED") || !strcmp(jobname1, "UHF")) {
    ...
    /*****JOB: EIGENVECTORS AND EIGENVALUES ***************************************************/

    if (!strcmp(jobname, "VECTORS_VALUES_09")) {

      int is_vec_val[3], bands_vec_val[2];
      int nktot, num_sym = symmetry->number_of_operators;
      //int nkunique, nktot, num_sym = symmetry->number_of_operators;
      KPOINT_TRAN knet;
      int_value[0] = 1;
      ...
      read_line(file.job, title, 99);
      sscanf(title, "%d %d %d %d %d", &is_vec_val[0], &is_vec_val[1], &is_vec_val[2], &bands_vec_val[0], &bands_vec_val[1]);

      read_line(file.job, title, 99);
      sscanf(title, "%s", file.directory1);

      if (job->taskid == 0 && job->C09 != 1) {
      fprintf(file.out,"Keyword READ_CRYSTAL_09 must be set to read data from CRYSTAL09\n");
      MPI_Finalize();
      exit(1);
     }

      knet_check(is_vec_val,jobname,crystal,file);
      int_range_check(bands_vec_val,int_value,int_range,jobname,file);

        if (job->taskid == 0) {
        fprintf(file.out,"===========================================================================================================\n");
        fprintf(file.out,"|                             CRYSTAL09 EIGENVECTOR AND EIGENVALUE CALCULATION                             |\n");
        fprintf(file.out,"===========================================================================================================\n");
        fprintf(file.out,"| MONKHORST-PACK   %2d %2d %2d | BAND RANGE %4d to %4d |                                                   |\n", \
        is_vec_val[0], is_vec_val[1], is_vec_val[2], bands_vec_val[0], bands_vec_val[1]);
        fprintf(file.out,"===========================================================================================================\n");
       }

      //knet_size(&nktot,is_vec_val,crystal);
      //count_k_points(num_sym,is_vec_val,&nkunique,crystal,symmetry,job,file);
      //count_k_points(is_vec_val,&nkunique,&nktot,crystal,symmetry,job,file);
      //allocate_k_points(&knet,nkunique,nktot,crystal,job,file);
      //knet.unique = nkunique;
      count_k_points(&knet,is_vec_val,crystal,symmetry,job,file);
      allocate_k_points(&knet,crystal,job,file);
      if (job->C09 == 1)
      read_XCBD_crystal_09(&knet,crystal,job,file);
      generate_k_points(&knet,is_vec_val,crystal,symmetry,job,file);
      write_vectors_write_values_rma3(is_vec_val, &knet, bands_vec_val, atoms, symmetry, crystal, job, file);
      //read_XCBD_crystal_09(nkunique,&knet,crystal,job,file);
      // change 6 generate_k_points(&knet,num_sym,is_vec_val,nkunique,crystal,symmetry,job,file);
      //write_vectors_write_values_rma3(is_vec_val, nkunique, &knet, bands_vec_val, atoms, symmetry, crystal, job, file);

      free_k_points(&knet,job);
    }

/*
    if (!strcmp(jobname, "VECTORS_VALUES")) {
      ...
      if (job->taskid == 0)
         {
        fprintf(file.out, "EIGENVECTOR AND EIGENVALUE CALCULATION\n");
      ...
      count_k_points(num_sym,is_vec_val,&nkunique,crystal,symmetry,job,file);
      allocate_k_points(&knet,nkunique,nktot,crystal,job,file);
      generate_k_points(&knet,num_sym,is_vec_val,nkunique,crystal,symmetry,job,file);
      write_vectors_write_values_rma1(is_vec_val, nkunique, &knet, bands_vec_val, atoms, job, file);

      free_k_points(&knet,job);

    }

    if (!strcmp(jobname, "VECTORS_VALUES_SHIFTED")) {

      int is_vec_val[3], is2[3], bands_vec_val[2];
      int nkunique, nkshifted, nktot, num_sym = symmetry->number_of_operators;
      KPOINT_TRAN knet, knet_shifted;
      int_value[0] = 1;
      int_value[1] = atoms->number_of_sh_bfns_in_unit_cell;
      int_range = "ENERGY BAND";
      ...
      is2[0] = 2 * is_vec_val[0];
      is2[1] = 2 * is_vec_val[1]; 
      is2[2] = 2 * is_vec_val[2];
      ...
      generate_k_points(&knet,num_sym,is_vec_val,nkunique,crystal,symmetry,job,file);
      write_vectors_write_values_rma1(is_vec_val, nkunique, &knet, bands_vec_val, atoms, job, file);
      //write_vectors_rma1(is_vec_val, nkunique, &knet, bands_vec_val, atoms, job, file);   FIX THIS ROUTINE
      count_shifted_k_points(num_sym,is_vec_val,&nkshifted,crystal,symmetry,job,file);
      allocate_k_points(&knet_shifted,nkshifted,nktot,crystal,job,file);
      generate_shifted_k_points(&knet_shifted,num_sym,is_vec_val,nkshifted,crystal,symmetry,job,file);
      write_values_rma1(is2, nkshifted, &knet_shifted, bands_vec_val, atoms, job, file);

      free_k_points(&knet,job);
      free_k_points(&knet_shifted,job);
...
*/
    ...
    if (!strcmp(jobname, "FERMI_SURFACE")) {

      int is_fermi[3], bands[2];
      int_value[0] = 1;
      int_value[1] = atoms->number_of_sh_bfns_in_unit_cell;
      //int_range = "ENERGY BAND";
      char int_range[12] = "ENERGY BAND";
      ...
      read_line(file.job, title, 99);
      sscanf(title, "%d %d %d %d %d", &is_fermi[0], &is_fermi[1], &is_fermi[2], &bands[0], &bands[1]);

      read_line(file.job, title, 99);
      sscanf(title, "%s", file.directory1);

      knet_check(is_fermi,jobname,crystal,file);
      int_range_check(bands,int_value,int_range,jobname,file);

      if (job->taskid == 0)
         {
        fprintf(file.out, "FERMI SURFACE CALCULATION\n");
        fprintf(file.out, "SHRINKING FACTOR %d %d %d\tLOWER BAND LIMIT %d\tUPPER BAND LIMIT %d\n\n", \
        is_fermi[0], is_fermi[1], is_fermi[2], bands[0], bands[1]);
         }

      ////fermi_surface(is_fermi,bands,crystal,symmetry,atoms,job,file);

    }

    /*****JOB: OPTICAL SPECTRUM ROOT SAMPLING METHOD *******************************/

    else if (!strcmp(jobname, "OPTICAL_SPECTRUM_ROOT")) {

      int is_opt[3], bands_opt[2];
      int npoints;
      double energy_range[2], field[3];
      int_value[0] = 1;
      int_value[1] = atoms->number_of_sh_bfns_in_unit_cell;
      //int_range = "ENERGY BAND";
      char int_range[12] = "ENERGY BAND";
      ...
      read_line(file.job, title, 99);
      sscanf(title, "%d %d %d %d %d", &is_opt[0], &is_opt[1], &is_opt[2], &bands_opt[0], &bands_opt[1]);

      knet_check(is_opt,jobname,crystal,file);
      int_range_check(bands_opt,int_value,int_range,jobname,file);

      read_line(file.job, title, 99);
      sscanf(title, "%d %lf %lf", &npoints, &energy_range[0], &energy_range[1]);

      read_line(file.job, title, 99);
      sscanf(title, "%lf %lf %lf", &field[0], &field[1], &field[2]);

      ////optical_spectrum(is_opt, bands_opt, npoints, energy_range, field);
...
*/
    ...
    fprintf(file.out,"         |\n");
     ...
     ////state_density(is_den,bands_den,npoints,nprojections,atom_proj,energy_range,density_type,atoms,atom_p,shells,gaussians,crystal,symmetry,R,R_tables,G,job,file);

    }

    /*****JOB: OPTICAL SPECTRUM TETRAHEDRON METHOD *********************************/

    else if (!strcmp(jobname, "OPTICAL_SPECTRUM")) {

      int is_den[3], bands_den[2];
      int npoints;
      double energy_range[2];
      //char optical_type = 'O';
      double_value[0] = lower_energy_limit;
      ...
      fld[i].comp3 /= fld_tmp;
      //if (job->taskid == 0) {
      //fprintf(file.out,"Angles (degrees) between field directions and Cartesian axes\n");
      //fprintf(file.out,"%5.2lf %5.2lf %5.2lf\n",acos(fld[i].comp1) / deg_rad, acos(fld[i].comp2) / deg_rad, acos(fld[i].comp3) / deg_rad);
     //}

     }

      ////susceptibility(is_den,bands_den,npoints,energy_range,fld,atoms,atom_p,shells,gaussians,crystal,symmetry,R,R_tables,G,job,file);
      //susceptibility1(is_den,bands_den,npoints,energy_range,fld,atoms,atom_p,atom_i,shells,gaussians,crystal,symmetry,R,R_tables,G,job,file);


    }

    /*****JOB: RPA DIELECTRIC FUNCTION *******************************************/

    else if (!strcmp(jobname, "DIELECTRIC_FUNCTION")) {

      int is_dfn[3], bands_dfn[2];
      int npoints;
      double energy_range[2];
      double_value[0] = lower_energy_limit;
      double_value[1] = upper_energy_limit;
      ...
      read_line(file.job, title, 99);
      sscanf(title, "%d %d %d %d %d", &is_dfn[0], &is_dfn[1], &is_dfn[2], &bands_dfn[0], &bands_dfn[1]);

      read_line(file.job, title, 99);
      sscanf(title, "%s", file.scf_eigvec);

      knet_check(is_dfn,jobname,crystal,file);
      int_range_check(bands_dfn,int_value,int_range,jobname,file);
      ...
      ////dielectric_function(is_dfn,bands_dfn,npoints,energy_range,atoms,atom_p,shells,gaussians,crystal,symmetry,R,R_tables,G,job,file);

    }

    /*****JOB: RAMAN SPECTRUM TETRAHEDRON METHOD *********************************/

    else if (!strcmp(jobname, "RAMAN_SPECTRUM")) {
      ...
      /////raman_suscep(is_den,bands_den,npoints,energy_range,fld,atoms,atom_p,atom_i,shells,gaussians,crystal,symmetry,R,R_tables,G,job,file);
   ...
   do {
      ...
      ////          }
       ...
       ////         }
        ...
        //HERE 22/05/20 if (!strcmp(jobname1, "SPIN")) {
           read_line(file.job, title, 99);
           ...
           //HERE sscanf(title, "%d", &job->spin_polarisation);
           //switch (job->spin_polarisation) {
              //case TRUE:
                //job->spin_dim = 2;
                //job->spin_fac = 1;
          ...
          //for (i = 0; i < atoms->number_of_atoms_in_unit_cell; i++) {
           ...
           //}
            ...
            for (i = 0; i < number_of_magnetic_ions; i++) {
              ...
              //fprintf(file.out,"ion %3d %3d %3d\n",number_of_magnetic_ions,ion-1,spin);
              spin_tot += spin;
             }
      ...
      read_line(file.job, title, 99);
      //CHANGESMAY2017char guess_atom[10];
    ...
    //}
      ...
      if (!strcmp(jobname1, "OVERLAP_TOL")) {
      read_line(file.job, title, 99);
      sscanf(title, "%lf %lf %lf %lf %lf %lf %lf %lf %lf", &job->overlap_tol_1, &job->overlap_tol_2, &job->overlap_tol_3, \
      &job->overlap_tol_4 ,&job->overlap_tol_5, &job->overlap_tol_6,  &job->overlap_tol_7, &job->overlap_tol_8, &job->overlap_tol_9);
      job->overlap_tol_1 = pow(10.00,-job->overlap_tol_1);
      job->overlap_tol_2 = pow(10.00,-job->overlap_tol_2);
      job->overlap_tol_3 = pow(10.00,-job->overlap_tol_3);
      job->overlap_tol_4 = pow(10.00,-job->overlap_tol_4);
      job->overlap_tol_5 = pow(10.00,-job->overlap_tol_5);
      job->overlap_tol_6 = pow(10.00,-job->overlap_tol_6);
      job->overlap_tol_7 = pow(10.00,-job->overlap_tol_7);
      job->overlap_tol_8 = pow(10.00,-job->overlap_tol_8);
      job->overlap_tol_9 = pow(10.00,-job->overlap_tol_9);

      //2019if (job->taskid == 0)
      //fprintf(file.out,"Lattice Tolerances \
      %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e\n",\
      job->overlap_tol_1,job->overlap_tol_2,job->overlap_tol_3,job->overlap_tol_4,job->overlap_tol_5,job->overlap_tol_6,\
      job->overlap_tol_7,job->overlap_tol_8,job->overlap_tol_9);

      if (job->overlap_tol_1 < 1.0e-24 || job->overlap_tol_1 > 1.0e-03 || \
          job->overlap_tol_2 < 1.0e-24 || job->overlap_tol_2 > 1.0e-03 || \
          job->overlap_tol_3 < 1.0e-24 || job->overlap_tol_3 > 1.0e-03 || \
          job->overlap_tol_4 < 1.0e-24 || job->overlap_tol_4 > 1.0e-03 || \
          job->overlap_tol_5 < 1.0e-24 || job->overlap_tol_5 > 1.0e-03 || \
          job->overlap_tol_6 < 1.0e-24 || job->overlap_tol_6 > 1.0e-03 || \
          job->overlap_tol_7 < 1.0e-24 || job->overlap_tol_7 > 1.0e-03 || \
          job->overlap_tol_8 < 1.0e-24 || job->overlap_tol_8 > 1.0e-03 || \
          job->overlap_tol_9 < 1.0e-24 || job->overlap_tol_9 > 1.0e-03) {
      if (job->taskid == 0)
      fprintf(file.out,"ERROR: Lattice Tolerances must be in range 1.0E-24 to 1.0E-03. Values entered \
      %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e %4.1e\n",\
      job->overlap_tol_1,job->overlap_tol_2,job->overlap_tol_3,job->overlap_tol_4,job->overlap_tol_5,job->overlap_tol_6,job->overlap_tol_7,\
      job->overlap_tol_8,job->overlap_tol_9);
      MPI_Finalize();
      ...
      if (!strcmp(jobname1, "EXACT_COULOMB_INTEGRALS")) {
      job->coul_int = 1;
      sprintf(integrals,"Ewald");
     }

      if (!strcmp(jobname1, "MULTIPOLE_COULOMB_INTEGRALS")) {
      job->coul_int = 2;
      // New Multipole ints job->coul_int = 3;
      sprintf(integrals,"Multipole");
     }
      ...
      if (!strcmp(jobname1, "OVERLAP_WEIGHTS") && job->scf_denfit == 1) {
      job->scf_dencou = 0;
      if (job->taskid == 0)
      fprintf(file.out,"USING 3-CENTRE OVERLAPS FOR DENSITY FITTING COEFFICIENTS\n");
     }
 ...
 fprintf(file.out,"-----------------------------------------------------------------------------------------------------------\n");
 if (crystal->type[0] == 'C' || crystal->type[0] == 'S' || crystal->type[0] == 'P') { fprintf(file.out,\
 ...
 fprintf(file.out,"-----------------------------------------------------------------------------------------------------------\n\n");
 //fprintf(file.out,"| LATTICE SUM TOLERANCES    %7.1e %7.1e %7.1e %7.1e %7.1e %7.1e %7.1e %7.1e %7.1e\n", \
 job->overlap_tol_1, job->overlap_tol_2, job->overlap_tol_3, job->overlap_tol_4 ,job->overlap_tol_5, job->overlap_tol_6, \
 job->overlap_tol_7, job->overlap_tol_8, job->overlap_tol_9);
 //fprintf(file.out,"-----------------------------------------------------------------------------------------------------------\n");
 ...
 //integrals_molecule_test(&fermi,atoms,atoms_ax,atom_p,shells,gaussians,shells_ax,gaussians_ax,crystal,symmetry,R,\
 R_tables,G,job,file);
 //orbital_product_density_fit_crystal_test(&fermi,atom_p,atoms,shells,gaussians,atoms_ax,shells_ax,gaussians_ax,crystal, \
 symmetry,R,R_tables,G,job,file);
 //integrals_crystal_coulomb_test(&fermi,atoms,atoms_ax,atom_p,shells,gaussians,shells_ax,gaussians_ax,crystal,symmetry,R,\
 R_tables,G,job,file);
 //integrals_crystal_exchange_test(&fermi,atoms,atoms_ax,atom_p,shells,gaussians,shells_ax,gaussians_ax,crystal,symmetry,R,\
 R_tables,G,job,file);
 //integrals_crystal_exchange_energy_test(&fermi,atoms,atoms_ax,atom_p,shells,gaussians,shells_ax,gaussians_ax,crystal,symmetry,R,\
 R_tables,G,job,file);
  ...
  switch (crystal->type[0]) {
   ...
   case 'P':
     ...
     //CHANGES2015 crystal_scf(&fermi, atoms, atom_p, shells, gaussians, crystal, symmetry, R, R_tables, G, job, file);
     break ;
   case 'M':
 ...
 free_fermi(&fermi,job);
    ...
    /*****JOB: GW CALCULATION ***************************************************************************************/
      ...
      double energy_range[2];
      char fermi_homo[12], fermi_bands[12], spectrum[24], monkhorst_pack[9];
      double_value[0] = lower_energy_limit;
      double_value[1] = upper_energy_limit;
      //double_range="ENERGY RANGE";
      char double_range[13] = "ENERGY RANGE";
...
//HERE       sprintf(fermi_homo, "       %d",fermi.homo);
    ...
    /*****JOB: BETHE-SALPETER EQUATION HAMILTONIAN DIAGONALISATION AND OPTICAL SPECTRUM *****************************/
      ...
      double energy_range[2];
      char optical_type = 'O';
      //char hamiltonian[12], monkhorst_pack[12], mpi_io[80], spectrum[14], fermi_homo[12], fermi_bands[12], field[80];
...
*/
   ...
   do {
     ...
     /*
      for (i = 0; i < numfrag; i++) {
      fprintf(file.out,"DIPOLE FRAGMENT %3d\n",i+1);
      for (j = 0; j < natoms[i]; j++) { 
      fprintf(file.out,"%3d ",nat[j][i] + 1);
     }
      fprintf(file.out,"\n");
     }
    */
...
/*
      ...
      read_line(file.job, title, 99);
      sscanf(title, "%s", file.directory1);
      sprintf(mpi_io,"%s",file.directory1);
    }
*/
   ...
   fflush(file.out);
        ...
        switch (crystal->type[0]) {
        case 'C':
        //bse_crystal(&fermi,atoms,atom_p,&numfrag,natoms,nat,shells,gaussians,atoms_ax,shells_ax,gaussians_ax,crystal, \
        symmetry,R,R_tables,G,job,file);
        bse_crystal1(&fermi,atoms,atom_p,&numfrag,natoms,nat,shells,gaussians,atoms_ax,shells_ax,gaussians_ax,crystal, \
        symmetry,R,R_tables,G,job,file);
        //optical_spectrum_crystal(&fermi,atoms,atom_p,&numfrag,natoms,nat,shells,gaussians,atoms_ax,shells_ax,gaussians_ax, \
        crystal,symmetry,R,R_tables,G,job,file);
        //optical_spectrum_crystal1(&fermi,atoms,atom_p,&numfrag,natoms,nat,shells,gaussians,atoms_ax,shells_ax,gaussians_ax, \
        crystal,symmetry,R,R_tables,G,job,file);
...
int i;
char fermi_homo[36], fermi_bands[9], monkhorst_pack[9];
