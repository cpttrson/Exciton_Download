...

{
  ...
  double gamma_1, gamma_1_inv, gamma_inv, small;
  ...
  double root_gamma_0 = sqrt(gamma_0);
    ...
    //CHANGES2015 F_cart = (double *) malloc(10000 * sizeof(double)); // 10000 = 10^4 for f functions
    if (F_cart == NULL) {
    ...
    //p = 0; q = 0; ip = 0; gi = 0; jp = 0; gj = 0; kp = 0; gk = 0; lp = 0; gl = 0;
    //fprintf(file.out,"ip jp kp lp    gi gj gk gl          %3d %3d %3d %3d   %3d %3d %3d %3d\n",ip,jp,kp,lp,gi,gj,gk,gl) ;
       ...
       case 'S':
            ...
            for (i4 = 0; i4 < shells->ng_sh[index_i] * shells->ng_sh[index_j]; i4++) {
              ...
              for (j4 = 0; j4 < shells->ng_sh[index_k] * shells->ng_sh[index_l]; j4++) {
                ...
                fac1 = sab[i4] * scd[j4];
//fprintf(file.out,"fac1 %3d %3d %20.10lf\n",i4,j4,fac1);
                p_inv_ex = pab_inv[i4] + pcd_inv[j4];
                p_fgtuv = fgtuv + i4 * mm4 + j4;
...
//fprintf(file.out,"fgtuv %3d %20.10lf\n",k,fac1 * two * pi / crystal->primitive_cell_volume * sum5[k]);
                }
                count = 1;
                for (index_S = 1; index_S < G->number_of_shells; index_S++) {
       ...
       //fprintf(file.out,"Done Electron-electron repulsion for 2-D system\n");
       break;
       ...
       if (job->taskid == 0)
       fprintf(file.out,"integrals_crystal_ijkl3 routine is for periodic systems only\n");
       MPI_Finalize();
       exit(1);
  ...
  for (i4 = 0; i4 < shells->ng_sh[index_i] * shells->ng_sh[index_j]; i4++) {
    ...
    for (j4 = 0; j4 < shells->ng_sh[index_k] * shells->ng_sh[index_l]; j4++) {
      ...
      fac1 = sab[i4] * scd[j4];
      count = 0;
      //for (index_S = 0; index_S < R->number_of_shells; index_S++) {
    ...
    for (j = 0; j < shelj1; j++) {
      for (k = 0; k < shelk1; k++) {
        for (l = 0; l < shell1; l++) {
          ...
          //fprintf(file.out,"INT %3d %3d %3d %3d %16.6e\n",bfposi1+i,bfposj1+j,bfposk1+k,bfposl1+l,*p_F_sh);
