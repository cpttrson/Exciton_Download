...
void integrals_crystal_ija(int p, TRIPLE_TRAN *triple, int *start_index, Complex *Coulomb, REAL_LATTICE *R, Q_LATTICE *q_G, RECIPROCAL_LATTICE *G, ATOM *atoms, SHELL *shells, GAUSSIAN *gaussians, ATOM *atoms_ax, SHELL *shells_ax, GAUSSIAN *gaussians_ax, CRYSTAL *crystal, JOB_PARAM *job, FILES file)
...
void three_centre_coulomb1_reversed2_crystal_test2(INTEGRAL_LIST_COMPLEX *integral_list, int p, TRIPLE_TRAN *triple, int *start_index, Complex *Coulomb, REAL_LATTICE *R, Q_LATTICE *q_G, RECIPROCAL_LATTICE *G, ATOM *atoms, SHELL *shells, GAUSSIAN *gaussians, ATOM *atoms_ax, SHELL *shells_ax, GAUSSIAN *gaussians_ax, CRYSTAL *crystal, JOB_PARAM *job, FILES file)
  ...
  for (i4 = 0; i4 < shells->ng_sh[index_i] * shells->ng_sh[index_j]; i4++) {
    for (j4 = 0; j4 < shells_ax->ng_sh[index_k]; j4++) {
      ...
      for (index_S = 0; index_S < R->number_of_ewald_shells; index_S++) {
       ...
       for (i = 0; i < sheli1; i++) {
         count_i = (bfposi1 + i) * nd5 * nd6;
         for (j = 0; j < shelj1; j++) {
           count_j = (bfposj1 + j) * nd6;
           for (k = 0; k < shelk1; k++) {
             count = count_i + count_j + bfposk1 + k;
             if (fabs((Coulomb[count]).real() * (Coulomb[count]).real() + (Coulomb[count]).imag() * (Coulomb[count]).imag()) > 1.0e-10) {
             integral_list->value[int_count] = Coulomb[count];
